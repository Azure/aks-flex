...
func init() {
	...
	Command.Flags().BoolVar(&deployGPUDevicePlugin, "nvidia-device-plugin", false, "install NVIDIA GPU Device Plugin via Helm")
	Command.Flags().BoolVar(&deployDRADriver, "nvidia-dra-driver", false, "install NVIDIA DRA Driver via Helm")
